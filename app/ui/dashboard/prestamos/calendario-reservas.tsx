...
export default function CalendarioReservas({ 
  ...
  useEffect(() => {
    const fetchEventos = async () => {
      if (!espacioId || !fechaSeleccionada) return;
      ...
      try {
        ...
        const eventosReservados = data.filter((evento: any) => evento.extendedProps?.tipo === 'reservado');
    ...
    fetchEventos();
  }, [espacioId, fechaSeleccionada]);

  // Generar eventos disponibles para demostración
  const generarEventosDisponibles = () => {
    if (!fechaSeleccionada) return;
    
    const eventosDisponibles: Evento[] = [];
    const horaInicio = 8; // 8:00 AM
    const horaFin = 18; // 6:00 PM
    
    // Crear bloques de 1 hora disponibles
